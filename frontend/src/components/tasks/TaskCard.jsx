...
export default function TaskCard({ task, onEdit, onDelete, onStartTimer, onToggleComplete, onPlanStudy }) {
    ...
    <Card className={[styles.card, isOverdue && styles.overdue, task.status === 'completed' && styles.done].filter(Boolean).join(' ')}>
      <div className={styles.top}>
        ...
        <div className={styles.actions}>
          ...
          {onPlanStudy && task.status !== 'completed' && (
            <Button variant="ghost" size="sm" onClick={() => onPlanStudy(task)}>＋ Plan</Button>
          )}
